...
public class Array5354 {
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
        ...
        return numOfMinutes(headID, map, informTime);
    }

    public int numOfMinutes(int headID, Map<Integer, List<Integer>> map, int[] informTime) {
        int max = 0;
        if (!map.containsKey(headID)) {
            return max;
        }
        for (Integer i : map.get(headID)) {
            max = Math.max(max, informTime[headID] + numOfMinutes(i, map, informTime));
        }
        return max;
    }
}
